...
def preprocess_training_data(sample_df):
...
def save_preprocessed_training_data(sample_df_preprocessed):
    loading.write_csv_from_path(sample_df_preprocessed, cst.PREPROCESSED_TRAIN_FILE_PATH)

def make_predictions_on_training_data(model, sample_df_preprocessed):
    """Add predictions to the preprocessed training dataset

    Args:
        model (Pipeline): trained model
        sample_df_preprocessed (pd.DataFrame): [description]

    Returns:
        [type]: [description]
    """
    sample_df_preprocessed_pred = sample_df_preprocessed.copy()
    sample_df_preprocessed_pred[cst.y_pred] = model.predict(sample_df_preprocessed.drop(cst.y_name, axis=1))
...
def save_predicted_training_data(sample_df_preprocessed_pred):
