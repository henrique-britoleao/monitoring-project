...
def graph_categorical_dist(
    ...
    pu.update_fig_centered_title(
        fig, f"Sample vs Batch Class Distribution in column {categorical_col}"
    )
...
def graph_categorical_dist_diff(
    ...
    """Graph to plot the difference in distribution between the sample and batch 
    data for a given column
    ...
    pu.update_fig_centered_title(
        fig,
        f"Sample vs Batch Class Distribution Difference in column {categorical_col}",
    )
    fig.update_layout(legend=dict(yanchor="top", xanchor="right",))
